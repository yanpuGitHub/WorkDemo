 ...
 * @description:
 */
public class CustomizeLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler {
    ...
    public boolean isAddFooter; // 是否有可以移动的脚布局
     ...
     * helps {@link CustomizeLayoutManager} make those decisions.
    ...
    /**
     * Works the same way as {@link AbsListView#setStackFromBottom(boolean)} and
     * it supports both orientations.
     * see {@link AbsListView#setStackFromBottom(boolean)}
     */
    private boolean mStackFromEnd = false;

    /**
     * Works the same way as {@link AbsListView#setSmoothScrollbarEnabled(boolean)}.
     * see {@link AbsListView#setSmoothScrollbarEnabled(boolean)}
     */
     ...
     */
    public CustomizeLayoutManager(Context context, boolean isAddFooter) {
     ...
     */
    public CustomizeLayoutManager(Context context, int orientation, boolean reverseLayout) {
     ...
     */
    public CustomizeLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                  int defStyleRes) {
    ...
    /**
     * Compatibility support for {@link AbsListView#setStackFromBottom(boolean)}
     */
    ...
    /**
     * Sets the orientation of the layout. {@link CustomizeLayoutManager}
     ...
     * {@link AbsListView#setStackFromBottom(boolean)}, use
     ...
     * By default, {@link CustomizeLayoutManager} lays out 1 extra page of
        ...
        LinearSmoothScroller linearSmoothScroller =
                new LinearSmoothScroller(recyclerView.getContext()) {
                    ...
                    public PointF computeScrollVectorForPosition(int targetPosition) {
                        return CustomizeLayoutManager.this.computeScrollVectorForPosition(targetPosition);
                    }
                };
...
//            }
        ...
        Log.d(TAG, "layoutForPredictiveAnimations scrapList = " + scrapList.size());
     ...
     * {@link ListView#setSelection(int)} or
     * {@link ListView#setSelectionFromTop(int, int)}, use
...
//            return 0;
//        }
        return hScrollBy(dx, recycler, state);
    }
...
//        }
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
        ...
        Log.d(TAG, "mLayoutState.mCurrentPosition = " + mLayoutState.mCurrentPosition);
        Log.d(TAG, "requiredSpace = " + requiredSpace);
    ...
    int hScrollBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        //位移0、没有子View 当然不移动
...
//        detachAndScrapAttachedViews(recycler);
        ...
        } else if (mHorizontalOffset + dx > totalWidth - getWidth()) {
            travel = totalWidth - getWidth() - mHorizontalOffset;
        }

        mHorizontalOffset += travel;
        int childCount = getChildCount();
        for (int i = 0; i < childCount; ++i) {
            View childAt = getChildAt(i);
            if (childAt != null) {
                if (isAddFooter && getPosition(childAt) == getItemCount() - 1) {
                    break;
                }
                childAt.offsetLeftAndRight(-travel);
            }
        }
        return dx;
    }
    ...
    private void recycleChildren(RecyclerView.Recycler recycler, int startIndex, int endIndex) {
        ...
        if (endIndex > startIndex) {
            for (int i = endIndex - 1; i >= startIndex; i--) {
                ...
                Log.d(TAG, "removeAndRecycleViewAt " + i);
            }
        } else {
     ...
     * independent from the rest of the {@link CustomizeLayoutManager}
        ...
        if (mOrientation == VERTICAL) {
            if (isLayoutRTL()) {
                ...
                if (getPosition(view) == getItemCount() - 1) {
                    ...
                    left = this.getPaddingLeft() - mHorizontalOffset;
                    right = this.mOrientationHelper.getDecoratedMeasurementInOther(view) - mHorizontalOffset;
                }
            }
            Log.e(TAG, "layoutChunk: 11111right: " + right + "   " + left + "       " + layoutState.mCurrentPosition);
            if (layoutState.mLayoutDirection == LayoutState.LAYOUT_START) {
...
//    }
    ...
    private View findFirstReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State
            state) {
        return findReferenceChild(recycler, state, 0, getChildCount(), state.getItemCount());
    }

    private View findLastReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State
            state) {
