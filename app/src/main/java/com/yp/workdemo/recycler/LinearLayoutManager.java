package com.yp.workdemo.recycler;
...
import com.yp.workdemo.recycler.RecyclerView.LayoutManager;
import com.yp.workdemo.recycler.RecyclerView.LayoutParams;
import com.yp.workdemo.recycler.RecyclerView.Recycler;
import com.yp.workdemo.recycler.RecyclerView.SmoothScroller.ScrollVectorProvider;
import com.yp.workdemo.recycler.RecyclerView.State;
import com.yp.workdemo.recycler.RecyclerView.ViewHolder;
...
public class LinearLayoutManager extends LayoutManager implements ViewDropHandler, ScrollVectorProvider {
    ...
    public LinearLayoutManager(Context context, boolean isHScroll) {
        this(context, 1, false);
        this.isHScroll = isHScroll;
    }
...
//        return ;
    ...
    int hScrollBy(int dx, Recycler recycler, State state) {
        //位移0、没有子View 当然不移动
...
//            Log.d(TAG, "scrollBy: consumed: " + consumed + "    " + this.mLayoutState.mScrollingOffset);
    ...
    private void recycleChildren(Recycler recycler, int startIndex, int endIndex) {
        if (startIndex != endIndex) {
            int i;
//            Log.e(TAG, "recycleChildren: startIndex: " + startIndex + "  " + endIndex);
            if (endIndex > startIndex) {
                for (i = endIndex - 1; i >= startIndex; --i) {
//                    Log.e(TAG, "recycleChildren: 11111: position: " + i );
                    this.removeAndRecycleViewAt(i, recycler);
                }
            } else {
                for (i = startIndex; i > endIndex; --i) {
//                    Log.e(TAG, "recycleChildren: 2222: position: " + i );
                    this.removeAndRecycleViewAt(i, recycler);
                }
            }
    ...
    private void recycleViewsFromStart(Recycler recycler, int dt) {
        if (dt >= 0) {
            int limit = dt;
            int childCount = this.getChildCount();
//            Log.e(TAG, "recycleViewsFromStart: childCount:  " + childCount);
            int i;
            View child;
            if (this.mShouldReverseLayout) {
...
//                        Log.e(TAG, "recycleViewsFromStart: 1111 : position: " + i);
                ...
                for (i = 0; i < childCount; ++i) {
                    child = this.getChildAt(i);
                    if (this.mOrientationHelper.getDecoratedEnd(child) > limit || this.mOrientationHelper.getTransformedEndWithDecoration(child) > limit) {
//                        Log.e(TAG, "recycleViewsFromStart: 2222 : position: " + i);
    ...
    private void recycleViewsFromEnd(Recycler recycler, int dt) {
        int childCount = this.getChildCount();
//        Log.e(TAG, "recycleViewsFromEnd: childCount:  " + childCount);
        if (dt >= 0) {
...
//                        Log.e(TAG, "recycleViewsFromEnd: 1111 : position: " + i);
                        this.recycleChildren(recycler, 0, i);
                        return;
                    }
                }
                ...
                for (i = childCount - 1; i >= 0; --i) {
                    child = this.getChildAt(i);
                    if (this.mOrientationHelper.getDecoratedStart(child) < limit || this.mOrientationHelper.getTransformedStartWithDecoration(child) < limit) {
//                        Log.e(TAG, "recycleViewsFromEnd: 2222 : position: " + i);
                        this.recycleChildren(recycler, childCount - 1, i);
                        return;
                    }
                }
...
//        Log.d(TAG, "fill: start: " + start);
        ...
        Log.e(TAG, "fill: layoutState: " + layoutState.mAvailable + "    " + layoutState.mExtra + "      " + layoutState.mInfinite);
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            layoutChunkResult.resetInternal();
            Log.e(TAG, "fill: mCurrentPosition: " + layoutState.mCurrentPosition);
...
//                    Log.e(TAG, "layoutChunk: addView");
                    this.addView(view);
                } else {
//                    Log.e(TAG, "layoutChunk: addView 0");
                    this.addView(view, 0);
                }
            } else if (this.mShouldReverseLayout == (layoutState.mLayoutDirection == -1)) {
                Log.e(TAG, "layoutChunk: addDisappearingView");
                this.addDisappearingView(view);
            } else {
                Log.e(TAG, "layoutChunk: addDisappearingView 0");
...
//                Log.e(TAG, "layoutChunk: isLayoutRTL: " + isLayoutRTL() + "     " + getDecoratedMeasuredWidth(view));

                if (this.isLayoutRTL()) {
                    right = getWidth() - this.getPaddingRight();
...
//                totalWidth = this.mOrientationHelper.getDecoratedMeasurementInOther(view);
                Log.e(TAG, "layoutChunk: 11111right: " + right + "   " + left + "       " + mHorizontalOffset);
                ...
                Log.e(TAG, "layoutChunk: 2222right: " + right + "   " + left);
            }
